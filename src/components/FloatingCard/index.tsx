            ...
            >
                <View>
                    <Text style={{ fontSize: 20 }}>
                        {spotInfo ? spotInfo.address.data.name : "Location Name"}
                    </Text>
                    <View style={{ display: "flex", flexDirection: "row" }}>
                        <Icon name="location" size={22} color="black" />
                        <Text style={{fontSize: 18, marginLeft: 3}}>{spotInfo ? spotInfo.address.data.address : "Location Address"}</Text>
                    </View>
                </View>
                <View >
                  <Pressable style={{marginBottom: 5, alignSelf: "flex-end"}} onPress={() => {
                    if(wishlist == false) {
                      bookmark_add(spotInfo.spotId);
                      setWishlist(true);
                    }else {
                      bookmark_delete(spotInfo.spotId);
                      setWishlist(false);
                    }
                  }}>
                    <Icon name={`heart${ wishlist? "" :"-outline"}`} size={30}/>
                  </Pressable>
                    <View
                        style={[
                ...
                <Animated.View layout={Layout} exiting={FadeOut} style={{ flex: 1 }}>
                    ...
                    >
                        ...
                        {posts.map((x, y) => (
                            ...
                            >
                                <Text style={{fontWeight: "bold", fontSize: 16}}>{x.userName}</Text>
                                <Text style={{fontSize: 16}}>{x.comment}</Text>
                            </View>
                        ))}
                    </ScrollView>
                    <View
